...
FILE_ROOT.parent
# 原始数据集
IMAGE_SET_ROOT = FILE_ROOT.joinpath(r"data_split")  # 图片区分文件的路径
IMAGE_PATH = FILE_ROOT.joinpath(r"bdd100k_img/train")  # 图片的位置
ANNOTATIONS_PATH = FILE_ROOT.joinpath(r"bdd100k_label/Annotations/train_filter_small_trun")  # 数据集标签文件的位置
LABELS_ROOT = FILE_ROOT.joinpath(r"bdd100k_label/yolo_label/train")  # 进行归一化之后的标签位置
...
def save_label_file(img_jpg_file_name, size, img_box):
    ...
    with open(save_file_name, "a+") as f:
        for box in img_box:
            if box[0] == 'car':  # 数据集 xml 中的 person 指的是头
                cls_num = 0
            elif box[0] == 'truck':
                cls_num = 0
            elif box[0] == 'bus':
...
if __name__ == '__main__':
    ...
    LABELS_ROOT.mkdir(exist_ok=True, parents=True)  # 建立 Label 文件夹
