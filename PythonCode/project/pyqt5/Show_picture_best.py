...
logfile = '/Users/lucifer/Code/PythonCode/project/pyqt5/log/log.json'
...
class MainWindow(QMainWindow):
    ...
    def intlog(self, path):
        
        self.ui.textBrowser.clear()

        with open(path, 'r', encoding='UTF-8') as f:     # 出现gbk解码问题 ‘r’ 后可加encoding='UTF-8'
            content = json.load(f)
    ...
    def update_logpath(self, path):
        ...
        if len(log_list) == self.logcnt:
            self.update_log(self.log_list[0])
            print('log update')
            print(self.log_list[0])
            print(self.logcnt)
            return 

        try:
            self.log_list = sorted(log_list, key=lambda x: os.path.getmtime(x), reverse=True)
        except Exception as ex:
...
if __name__=='__main__':
    ...
    logfile = settings.value('Path/log')
